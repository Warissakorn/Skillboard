...
**สถานะ:** รุ่นปรับปรุง UI อยู่ระหว่างตรวจรับบน Chrome ก่อนเผยแพร่

## Screenshots (UI รุ่นก่อนปรับปรุง)
...
## ทดสอบ UI โดยไม่แตะข้อมูลจริง

```bash
node --test tests/*.test.cjs
node .optimize/baseline/measure.cjs
```

ใช้ Node.js 24 และ Chrome API/DOM จำลอง: ทดสอบค้นหา ฟอร์มเพิ่ม/แก้ไข
ตัวเลือก Import การป้องกันบันทึกซ้ำ และการอ่านเนื้อหาเต็ม ไม่ทดแทนการทดสอบ
extension จริงใน Chrome ดูผลและรายการตรวจรับใน `.optimize/summary.md`

## โลโก้

ไอคอนแถบ Extension ใช้รูปเทป Prompt สีขาวบนพื้นน้ำเงินเข้ม เพื่อให้มองเห็นชัด
ทั้ง toolbar สีสว่างและมืด มีไฟล์ใช้งาน 16, 48 และ 128px และต้นฉบับ 1024px
ที่ `icons/icon-source.png` โลโก้เดียวกันแสดงในหัวหน้าต่าง

## สีและข้อความบนการ์ด

ใช้โทนน้ำเงินเข้ม–เทาอ่อน และกรมท่าในธีมมืด ปุ่มบนการ์ดเล็กกว้างตามข้อความ
และขึ้นแถวใหม่เมื่อพื้นที่ไม่พอ โดยยังคงขนาดตัวหนังสือเดิม

## โหมดการ์ดเล็กแนวนอน

กดปุ่ม **▥** ที่หัวหน้าต่างเพื่อสลับเป็นการ์ดแนวตั้งกว้าง 164px เรียงข้างกัน
เลื่อนด้วยแถบแนวนอนด้านล่าง หรือโฟกัสพื้นที่รายการแล้วใช้ปุ่มลูกศร
Popup จะเตี้ยลงจาก 580px เป็น 380px กด **☰** เพื่อกลับรายการแบบเดิม
ใช้งานได้ในหน้าต่างย่อด้วย: ขอปรับความสูงหน้าต่างรวมกรอบระบบเป็น 420px
และคืนขนาดก่อนหน้าเมื่อสลับกลับ โดยขนาดพื้นที่ภายในขึ้นอยู่กับระบบปฏิบัติการ

## หน้าต่างย่อ

กด **หน้าต่างย่อ ↗** บนแถบหัวเพื่อเปิดหน้าต่างแยกขนาดเริ่มต้น 360 × 480px
ลากขอบหน้าต่างเพื่อปรับขนาดได้ และคลิกกลับไปทำงานบนเว็บได้โดยหน้าต่างไม่ปิดเอง
ปุ่ม **ใช้ Prompt** ส่งไปยังแท็บที่เลือกอยู่ในหน้าต่างเบราว์เซอร์ที่ใช้เปิดหน้าต่างย่อ
หากปิดหน้าต่างต้นทางแล้ว ให้เปิดหน้าต่างย่อใหม่จากหน้าแชทที่ต้องการ
หน้าต่างนี้ไม่ได้บังคับอยู่เหนือแอปอื่นตลอดเวลา สามารถย่อ/ปิดด้วยปุ่มของระบบได้

หัวหน้าต่างไม่มี tagline และข้อความหัวซ้ำ ปุ่มรายการสูง 28px ช่องค้นหาสูง 36px
คงขนาดข้อความหลัก 16px และชื่อ Skill 17px รายการอัปเดตเมื่อข้อมูลเปลี่ยนจากอีกหน้าต่าง

## ฟีเจอร์

- ตัวหนังสือหลัก 16px ชื่อ Skill 17px และข้อความรอง/ปุ่ม 14px ไม่มีหมวดหมู่ใน UI
- หน้าหลักเน้นค้นหาและเลือกใช้ Prompt พร้อมธีมสว่าง/มืดอัตโนมัติ
- **อ่านเนื้อหาเต็ม** โดยไม่ต้องเปิดฟอร์มแก้ไข
- **เพิ่ม/แก้ไข/ลบ** skill ผ่านฟอร์ม (ชื่อ, เนื้อหา) พร้อม validation
  ภาษาไทย — เพิ่มผ่านปุ่ม **＋ เพิ่ม Skill** และแก้ไขโดยกด **แก้ไข** ที่รายการ ฟอร์มจะโหลดค่าเดิมขึ้นมาให้แก้แล้วอัปเดต
  โดยไม่สร้างรายการซ้ำ
- **ใช้ Prompt ↗** — ส่งเนื้อหา skill ไปแทรกในช่องแชทของแท็บ AI ที่เปิดอยู่ทันที
...
- **ค้นหาแบบ realtime** (debounce 150ms) ครอบคลุมชื่อ เนื้อหา และ tags
  พร้อมตัวนับผลลัพธ์ "พบ X skills"
- **Export/Import** เป็นไฟล์ `.json` — import เลือกได้ว่าจะแทนที่ข้อมูลเดิมทั้งหมด
  หรือรวมกับข้อมูลเดิมแบบไม่ซ้ำ id; ปุ่มยกเลิกไม่แก้ข้อมูล
...
| ข้อมูลหายหลัง Import | ตรวจว่าเลือกโหมดถูกต้องตอนนำเข้า (รวมกับข้อมูลเดิม / แทนที่ทั้งหมด / ยกเลิก) |
...
```bash
zip -r skilltape-v0.1.0.zip . -x "*.test.js" -x ".git/*" -x "*.zip" -x "tests/*" -x ".optimize/*"
```
